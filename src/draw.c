...
/*   Updated: 2019/01/16 14:32:55 by tbeguin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		test_segment(t_mlx *mlx_all, int x, int y, int color)
{
	...
	bres = ft_new_bres(mlx_all->win->x_ligne, mlx_all->win->y_ligne, x, y);
	if (bres->dx != 0 && bres->dx > 0 && bres->dy != 0 && bres->dy > 0)
	{
		if (bres->dy > 0)
		{
			if (bres->dx >= bres->dy)
			{
				...
				bres->dx *= 2;
				bres->dy *= 2;
				while (bres->cor1->x != bres->cor2->x)
				{
					mlx_pixel_put(mlx_all->mlx_ptr, mlx_all->win->win_ptr,
							bres->cor1->x, bres->cor1->y, color);
					bres->cor1->x++;
					if ((bres->e -= bres->dy) < 0)
					{
						bres->cor1->y++;
			...
			{
				bres->e = ft_abs(bres->dy);
				bres->dy *= 2;
				bres->dx *= 2;
				while (bres->cor1->y != bres->cor2->y)
				{
					mlx_pixel_put(mlx_all->mlx_ptr, mlx_all->win->win_ptr,
							bres->cor1->x, bres->cor1->y, color);
					bres->cor1->y++;
